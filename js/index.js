...
(function () {
 ...
 const months=['januari','februari','maret','april','mei','juni','juli','agustus','september','oktober','november','desember'],monthMin = ['','','','','','','','','','','',''],days = ['minggu','senin','selasa','rabu','kamis','jumat','sabtu'],daysMin = ['','','','','','',''],seasons = ['musim dingin','musim semi','musim panas','musim gugur'];function postDate(daysName, daysMinName, monthsName, monthsMinName, seasonsName) {const _counterLength = 10;for (let counter = 0; counter < _counterLength; counter++) {innerDate(counter, 'date-');innerDate(counter, 'date')} function innerDate(counter, dateType) {let newCounter;dateType === 'date-' ? newCounter = -counter : newCounter = counter; const _msInDay = 86400000, _localDate = new Date(Date.now() + (newCounter * _msInDay)), _day = _localDate.getDate(), _month = _localDate.getMonth() + 1, _year = _localDate.getFullYear(); const dayDefault = addZero(_day), monthDefault = addZero(_month), defaultDate = dayDefault + '.' + monthDefault + '.' + _year; const dateClass = dateType + counter, nodeList = document.querySelectorAll('.' + dateClass); for (let i = 0; i < nodeList.length; i++) {const dateFormat = nodeList[i].dataset.format;dateFormat !== undefined && dateFormat !== ''? nodeList[i].innerHTML = String(changeFormat(dayDefault, monthDefault, _year, dateFormat, newCounter)): nodeList[i].innerHTML = defaultDate} } function changeFormat(_day, _month, _year, format, counter) { let innerFormat = format; const testFormat = ["dd","mm","yyyy","monthFull"], dateFormat = { dd: _day, mm: _month, yyyy: _year, monthFull: getMonthName(_month, monthsName, true), }; for (let i = 0; i < testFormat.length; i++) { let string = testFormat[i]; let regExp = new RegExp(string); innerFormat = innerFormat.replace(regExp, dateFormat[string]); } return innerFormat.split(' ').join(' ') } function getMonthName(_month, monthsName, bigFirstLetter, counter) { const monthCounter = !!counter ? counter : 0, month = _month + monthCounter > 12 ? monthCounter - (12 - _month) : _month + monthCounter; return changeFirstLetter(bigFirstLetter, monthsName[month - 1]) } function addZero(numb){return numb<10?'0'+numb:numb} function changeFirstLetter(isBig,str){return isBig&&str.length>0?str[0].toUpperCase()+str.slice(1):str} }if (document.body.classList.contains('ev-date')) {document.addEventListener("DOMContentLoaded", function () {postDate(days, daysMin, months, monthMin, seasons)});}
      ...
      if (counter == 1) {
        $(".online-people p")
          .fadeOut(1100, function () {
            $(this).text("116 penjualan per jam");
          })
          .fadeIn();
        counter = 0;
      } else {
        $(".online-people p")
          .fadeOut(1100, function () {
            $(this).text("Sekarang ada " + randNum + "  orang di situs");
          })
          .fadeIn();
        counter = 1;
      }
  ...
  var bad = function () {
    $(".bad__bottom").hover(function () {
      $(".go-to").addClass("active");
    });
    ...
    $(".hover-left").hover(
      function () {
        $(".composition__ingredient--left").addClass("active");
      },
      function () {
        $(".composition__ingredient--left").removeClass("active");
      }
    );
    $(".hover-right").hover(
      function () {
        $(".composition__ingredient--right").addClass("active");
      },
      function () {
        $(".composition__ingredient--right").removeClass("active");
      }
    );
    $(".promo__info").hover(
      function () {
  ...
  var banner = function () {
    $(window).on("scroll resize", function () {
			hideBannerScroll(".x_order_form, .footer"); // сюда через кому дописываем классы блоков, которые должны скрыть баннер
  ...
  var composition = function () {
    $(".composition__btn").on("click", function () {
      // $(this).parent().siblings().removeClass("active");
      $(this).parent().parent().toggleClass("active");
    });
    var windowHeight = $(window).height();
    $(document).on("scroll", function () {
      $(".composition__content").each(function () {
        var self = $(this),
          height;
...
inputFile.change(function (e) {
    ...
    $(".feedback-btn").click(function () {
      ...
      var cityTest = pattern.test($(".input__val-3").val());
			fileText.html("Unggah foto Anda");
			fileIcon.hide();
...
function delimiter(price) {
  for (let i = 0; i < price.length; i++) {
    price[i].textContent = price[i].textContent.replace(
      /(\d)(?=(\d{3})+(\D|$))/g,
      "$1."
    );
  }
